											...
											@foreach($projects as $project)
												<div class="col-md-6" style="padding-left: 3px; padding-right: 3px !important;">
													<div style="font-size: 13px !important; background-color: #f4f4f4 !important; border-radius: 5px !important; padding-top: 10px; padding-bottom: 10px" class="col-md-12">
														...
														<span style="text-decoration:underline !important">Bids:</span>
														<span style="float: right !important">{{$project->bids->count()}}</span><br>
														<?php
														$bids = $project->bids;
														$totalAmount = 0;
														foreach($bids as $bd)
														{
															$totalAmount = $totalAmount + $bd->bid_amount;
														}
														?>
														<span style="text-decoration:underline !important">Average Bid:</span>
														@if($bids->count()>0)
															<span style="float: right !important">(ZMW){{number_format(($totalAmount/($bids->count())), 2, '.', ',')}}</span><br>
														@else
															<span style="float: right !important">(ZMW){{number_format((0), 2, '.', ',')}}</span><br>
														@endif
												...
												@if($i++%2==1)
													<div class="col-md-12" style=" height: 10px !important; font-size: 10px !important">
													&nbsp;
													</div>
												@endif
											@endforeach
